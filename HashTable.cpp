...
void HashTable::addData(const std::string& county, const std::string& date, const WeatherData& data) {
    weatherMap[{county, date}] = data;
}

WeatherData HashTable::getData(const std::string& county, const std::string& date) const {
    auto it = weatherMap.find({county, date});
    if (it != weatherMap.end()) {
        return it->second; // Return the pair of temperature and precipitation
    } else {
        return {0.0, 0.0}; // Return default values if data not found
    }
}
...
bool HashTable::isDouble(const string& str) { // Checks if the current token can be converted to a double or not
    stringstream ss(str);
    double d;
    ss >> d;
